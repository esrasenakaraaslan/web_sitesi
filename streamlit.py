...
if st.button("Makale 1"): 
    ...
    İstanbul ve Ankara'nın yanı sıra bu şehirlere yakın illerde de bilişim sektöründe önemli istihdam olanakları bulunmaktadır 
...
if st.button("Hakkımızda"): 
    ...
    Bilişim sektörü, sürekli değişen ve gelişen bir alan olmaya devam edecektir. FreshData gibi platformlar, bu değişime ayak uydurmak isteyenler için değerli bir kaynak oluştur 

ur. Veri analizi ve yenilikçi yaklaşımlarıyla, bilişim sektörünün geleceğini şekillendirmeye yardımcı olurlar. 

  

    #### FreshData'nın Avantajları 

  

    - **Güncel İş İlanları**: FreshData, sürekli güncellenen iş ilanlarıyla kullanıcılarına en güncel kariyer fırsatlarını sunar. 

    - **Meslek Analizleri**: Bilişim sektöründeki meslek grupları ve pozisyonlarıyla ilgili derinlemesine analizler yapar, kullanıcılara sektör hakkında detaylı bilgi sağlar. 

    - **Kullanıcı Dostu Arayüz**: Platformun kolay kullanılabilir arayüzü sayesinde kullanıcılar, istedikleri bilgilere hızlıca erişebilir ve aradıkları iş ilanlarını filtreleyebilirler. 

    - **Veri Odaklı Yaklaşım**: FreshData, veri analizi ve uzman görüşlerine dayanarak kullanıcılarına sektördeki gelecek trendleri hakkında bilgi sunar. 

    - **Özelleştirilebilir İçerik**: Kullanıcılar, ilgi alanlarına ve meslek gruplarına göre özelleştirilmiş içerikleri görüntüleyebilir ve takip edebilirler. 
    ...
    1. **Kayıt Olun**: FreshData platformuna kaydolarak kişisel profilinizi oluşturun ve ilgi alanlarınızı belirtin. 

    2. **İş İlanlarını İnceleyin**: Güncel iş ilanlarını ve sektördeki meslek analizlerini görüntüleyin. 

    3. **Meslek Gruplarına Göz Atın**: Bilişim sektöründeki farklı meslek gruplarına ve pozisyonlara göz atarak kariyeriniz için doğru adımları belirleyin. 

    4. **Özelleştirin ve Takip Edin**: İlgi alanlarınıza göre özelleştirilmiş içerikleri takip edin ve geleceğe yönelik trendleri takip edin. 
    ...
    FreshData platformunu daha yakından tanımak ve bilişim sektöründeki kariyerinizde doğru adımları atmak için bize ulaşın: 

  

    - **E-posta**: info@freshdata.com 

    - **Telefon**: 123-456-7890 

    - **Adres**: FreshData Inc. 1234 Teknoloji Cad. İstanbul, Türkiye 
    ...
    FreshData © 2024. Tüm hakları saklıdır. 
